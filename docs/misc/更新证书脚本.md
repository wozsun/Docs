...
访问方式中`编程访问`必选，用户权限只需选择`QcloudSSLFullAccess`和`QcloudCDNFullAccess`/`QcloudTEOFullAccess`两项即可，其余选项按需填写。
...
:::note
这是一个更新脚本而不是部署脚本，所以如果你的证书列表中没有正在使用的证书，脚本会报错。记得先上传一份证书并部署到云资源上，然后再按步骤操作。
:::
